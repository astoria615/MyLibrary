...
namespace MyLibrary.Controllers
{
    public class LibrarianController : Controller
    {
        ...
        private bool IsLibrarian()
        {
            ...
            return user != null && (user.Role == "Librarian" || user.Role == "Admin");
        }
        ...
        private List<BorrowActivityItem> BuildActivityList(List<int> borrowingIds)
        {
            ...
            foreach (var id in borrowingIds)
            {
                ...
                foreach (var d in details)
                {
                    ...
                    var cover = book != null ? db.BookImages.Where(i => i.BookId == book.BookId && i.IsPrimary).Select(i => i.ImageUrl).FirstOrDefault() : null;
        ...
        {
            if (!IsLibrarian()) return RedirectToAction("Login", "Account");
            RefreshSession();
        ...
        {
            if (!IsLibrarian()) return Json(new { success = false });
            int userId = int.Parse(User.Identity.Name);
            ...
            if (borrow == null) return Json(new { success = false, message = "Not found." });
        ...
        public ActionResult FineManagement()
        {
            ...
            // All active borrowings that have issues (overdue OR have damaged/lost copies)
            var overdueIds = db.Borrowings
                .Where(b => b.Status == "Overdue" ||
                            (b.ReturnDate == null && b.DueDate < DateTime.Now && b.Status == "Approved"))
                .Select(b => b.BorrowingId)
                .ToList();

            // Also get borrowings that have damaged/lost book statuses
            ...
            var allIssueIds = overdueIds.Union(damagedBorrowIds).Distinct().ToList();
        ...
        {
            if (!IsLibrarian()) return Json(new { success = false, message = "Unauthorized" });
            ...
            var readerUser = reader != null ? db.UserAccounts.FirstOrDefault(u => u.UserId == reader.UserId) : null;
            if (readerUser != null)
            {
                var notification = new EmailNotification
                {
                    ...
                    Body = string.Format("A {0} fine of ${1:F2} has been issued. Reason: {2}", fineType, amount, notes ?? "N/A"),
                    Status = "Sent",
                    SentAt = DateTime.Now,
        ...
        public ActionResult UpdatePaymentStatus(int fineId, string status)
        {
            ...
            if (fine == null) return Json(new { success = false, message = "Fine not found." });
        ...
        public ActionResult MarkPaid(int fineId, string paymentMethod, string transactionId, string paymentDate)
        {
            ...
            fine.PaymentDate = string.IsNullOrEmpty(paymentDate) ? DateTime.Now : DateTime.Parse(paymentDate);
            db.SubmitChanges();
        ...
        public ActionResult ReportIssue()
        {
            ...
            var books = db.Books.Where(b => b.IsActive)
                .Select(b => new { b.BookId, b.Title }).ToList();
            ViewBag.Books = new SelectList(books, "BookId", "Title");

            // Use a concrete type instead of anonymous
        ...
        public ActionResult ReportIssue(int bookId, int copyNumber, string issueType, string notes, int? readerId)
        {
            ...
            var copy = db.BookStatus.FirstOrDefault(s => s.BookId == bookId && s.CopyNumber == copyNumber);
            if (copy != null)
            {
                ...
                if (borrowing != null)
                {
                    ...
                    if (readerUser != null)
                    {
                        db.EmailNotifications.InsertOnSubmit(new EmailNotification
                        {
                            ...
                            Body = string.Format("A {0} fine of ${1:F2} has been issued. {2}", issueType, fineAmount, notes ?? ""),
                            Status = "Sent",
                            SentAt = DateTime.Now,
                            CreatedAt = DateTime.Now
                        });
        ...
        public ActionResult GetCopies(int bookId)
        {
            var copies = db.BookStatus
                ...
                .ToList()
                .Select(s => new { copyNumber = s.CopyNumber, status = s.Status })
                .ToList();
        ...
        public ActionResult GetAllReaders()
        {
            var readers = (from r in db.Readers
                           join u in db.UserAccounts on r.UserId equals u.UserId
                           select new { readerId = r.ReaderId, fullName = u.FullName })
                           .ToList();
